...
if __name__ == "__main__":
  start = datetime.now()
  dataset = ray.data.read_text("images_to_download.txt", concurrency=10)
  dataset = dataset.map(get_image, concurrency=10)
  ...
  dataset = dataset.map(Predictor, fn_constructor_args=[model_ref, processor_ref], concurrency=10)
  dataset = dataset.map(write_results, concurrency=10)
  dataset.materialize()
  end = datetime.now()
